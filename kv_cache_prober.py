...
def _generate_nonce_phrase() -> str:
    ...
    """
    nonce = secrets.token_hex(6)  # 12-char hex = 2^48 possibilities
    templates = [
        f"For context, our project ID is {nonce}.",
        f"The internal tracking number is {nonce}.",
        f"Our team uses reference code {nonce} for this.",
        f"The sprint identifier is {nonce}.",
        f"This relates to ticket {nonce}.",
        f"We track this under code {nonce}.",
    ]
